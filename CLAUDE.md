...
**每次修改 HTML 时，顺手根据系统当前时间折叠已过去的内容：**

- **当天已过去的时间节点**（结束时间 ≤ 当前时间）→ 给对应 `tl-item` 加 `collapsed` class，例如 `<div class="tl-item tl-ocean collapsed">`
- **整天已过去**（当前日期 > 该天日期）→ 像 D0 那样用 `<details class="dayX-details">` 包住整个 `<section>`，默认折叠，可点击展开回忆
- 先用 `date` 命令获取系统当前时间，再判断哪些节点需要折叠
